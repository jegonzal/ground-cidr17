...
\begin{scriptsize}
\begin{multicols}{2}
\lstinputlisting{groundapi.java}
\end{multicols}
\end{scriptsize}
\caption{Member variables for Common Ground Java classes. Methods for the classes have been elided. Full code is available at \texttt{https://github.com/ground-context/ground}.}
\label{fig:java}
\end{figure*}
...
The Common Ground metamodel is based on a layered graph structure shown in Figure~\ref{fig:layers}: one layer for each of the ABCs of data context. Skeletons of Java classes from the Ground codebase are shown in Figure~\ref{fig:java}.
...
% \end{multicols}
% \end{scriptsize}
% \caption{\VersionGraph metamodel in Scala.}
% \label{fig:core}
% \end{figure}
% \vikram{Rewrote this section and flipped the order of definitions to avoid redundancy.}

We begin with the \versiongraph layer of Common Ground, which captures changes corresponding to the \emph{C} in the ABCs of data context. 
...
To allow customization, the \versiongraph allows \version objects to be associated with
...
% \end{multicols}
% \end{scriptsize}
% \caption{\ModelGraph metamodel in Scala.}
% \label{fig:mantle}
% \end{figure}
...
The \modelgraph level of Common Ground provides a model-agnostic representation of application metadata: the \emph{A} of our ABCs. 
...
The \modelgraph is based on an internal superclass called \thing, which is simply a unique ID and a \kw{VersionHistoryDAG}. Note that 
...
To that end, the \modelgraph includes a subclass of \thing called {\structure}. A \structure is like a schema: a set of {\gtag}s that must be present. Unlike database schemas, the \structure class of Ground is versioned, via a \kw{StructureVersion} subclass in the \versiongraph.  If an \thing is associated with a \structure, each \version of the \thing is associated with a corresponding \kw{StructureVersion} and must define those {\gtag}s (along, optionally, with other ad hoc {\gtag}s.)
...
\smallitem{LazyBundles: Compressed Relationships}
The Common Ground model captures versions of relationships (e.g., \edge{}s) between versioned objects (e.g., \node{}s). Note that the relationships themselves are first-class objects with identity and tags. Implemented naively, the version history of these relationship objects can grow in unintended and undesirable ways. We address that problem by underlaying the logical Common Ground model with a physical compression scheme combined with lazy materialization of logical versions.
...
We can remove the fanout factor using what we call a \kw{LazyBundle}, an implementation detail that does not change the logical metamodel of Common Ground. In essence, \kw{LazyBundle}s are compressed sets of \kw{EdgeVersion}s that all have one of their two endpoints in common. As an example, we return to a version $m_v$ of node $M$ and a version $n_w$ of one of the adjacent nodes $N_k$. If we change $M$ 100 times before we change $N$, there will be 100 logical \kw{EdgeVersion}s connecting $n_w$ to $m_v, \ldots, m_{v+100}$. Rather than eagerly materializing those \kw{EdgeVersion}s explicitly, we can store a \kw{LazyBundle} capturing the relationship between $w$ and the range $[v, v+100]$. More generally, in a branching version history, a \kw{LazyBundle} connects a single version of one item to a rooted subgraph of the versions of another item. We can use \kw{LazyBundle}s for any relationship between versioned objects: the \edge{}s between \node{}s, the membership of \node{}s and \edge{}s in \graph{}s, etc.
...
% \end{multicols}
% \end{scriptsize}
% \caption{\LineageGraph metamodel in Scala.}
% \label{fig:crust}
% \end{figure}
The goal of the \lineagegraph layer is to capture usage information composed from the nodes and edges in the model graph.  
