...
\section{Future Work}
Ground is a deliberately simple system, defined by the Common Ground API and metamodel. In the spirit of agility, we view it as a work in progress. However, our hope is that the simplicity of the Ground model and architecture will allow enable evolution and innovation to happen outside the system core.

\begin{itemize}
	\item Raising the API closer to application needs:
	\begin{itemize}
		\item "orbits" for relational and others
		\item ER/ORM-style specs rather than code
	\end{itemize}
	\item Storage systems. New class of DBMS! Also efficiency of versioned graph representations; SuperVersions are just one idea/example.
	\item Connect to/enhance ongoing aboveground work in the community:
	\begin{itemize} 
		\item schema extraction: both automation and wrangling. Affection for DSLs: synthesis and UX.
		\item entity extraction and related structure imposition
		\item data wrangling and cleaning
		\item data catalogs and profiling.
		\item visualization and chart recommendations
	\end{itemize}
	\item Network effects. 
	\begin{itemize}
		\item The social network power of data organizations. Many have said it, but it has yet to really happen. Why? What's the Utility! Empower the apps above!
		\item Multiplicative effects of different apps!
	\end{itemize}
	\item Reproducibility and governance. Combination of code and data issues. Connection to containers, VMs, orchestration scripts. Versioned data a la MIT work.
\end{itemize}
