...
For example, an application can subscribe for file crawl events, hand off the files to an entity extraction system like OpenCalais~\cite{opencalais} or DeepDive~\cite{deepdive}, and subsequently tag the corresponding Common Ground metadata objects with the extracted entities.
...
% % To support such lazy evaluation, \emph{ephemeral} \kw{LineageEdge}s can be generated in the Ground lineage API without materializing the (redundantly computed) \kw{LineageEdge}s in immutable storage. 
...
\label{sec:relwork}
Work related to this paper comes in a variety of categories, much of it based out of industry and open source, and not well documented in the research literature.
...
Both provide graph models that inspired the Common Ground \modelgraph. They are both focused on the specifics of
running jobs in today's Big Data stack. Neither provides versioning or the ability to easily integrate various kinds of models including both data and code. Based on what we hear in the field, neither is perceived as vendor-neutral. % These systems are also exploring ``underground'' alternatives for storage.
...
OpenChorus~\cite{openchorus} and IBM LabBook~\cite{labbook} provide overarching portals for collaboration on data projects.
LinkedIn WhereHows~\cite{wherehows}, FINRA Herd and Google Goods~\cite{goods} are metadata services built to support the evolving data workflows in their respective organizations. Goods is a particularly mature system, bundling what we call underground services with various proprietary services we might describe as aboveground applications.  Vistrails~\cite{vistrails} is an open source scientific workflow and provenance management system that shares some of our goals, with a focus on scientific reproducibility. Like many of the other systems mentioned here, it is designed for use in a particular style, rather than as a standalone context management system in a decoupled stack. However many of the issues explored in the work are good aboveground examples of what Ground needs to support
