...
\vikram{Like we talked about yesterday, should we flip this and talk about the traditional database stack instead of introducing big data first?} 
...
One negative side-effect of this diversity is the lack of an agreed-upon service
% for shared metadata 
%\msd{metadata or data context?} 
for these diverse components to assemble a collective metadata ``signature'' or ``paper trail'' of their content and behavior.
The only widely-deployed option is the Hive Metastore, which serves simple relational schemas---a dead end for representing the Variety of Big Data. As a result, most ``Data Lake'' projects lack 
even the most rudimentary metadata about their components' behavior.
...
``Tribal knowledge'' is a common description for how organizations manage this productivity problem; that is clearly not a systematic solution. \vikram{Should we also mention that it scales poorly?}
...
\vikram{``Change context'' sounds weird to me. Instead of calling each one context, can we say the ABCs of data context are application metadata, behavior and usage, and change over time? (or something like that).}
...
\smallitem{B: Behavioral Context}: This is information about how data was created and used over time. 
In the new decoupled stack, this lineage is often both polyglot and high-volume. \vikram{Should we add something about machine-generated in order to emphasize high-volume?} It comes from a diverse set of applications, languages and compute frameworks, varying in granularity and interpretability.
...
\smallitem{C: Change Context}: This is information about the version history of data objects over time, including changes to both the structure and content of the objects. Traditionally, metadata was focused on the present, but historical context is increasingly useful in modern agile organizations.
...
Finally, to join the social media names against the customer names, Preppy uses previous transformation scripts in Ground to suggest standardized join keys to Sue. \vikram{1. Shouldn't this be Janet and not Sue? 2. saying that transformation scripts are \textit{in} Ground sounds weird because it seems like Ground is a code repo.}
...
The latest context may not always be the most relevant: we may want to replay scenarios from the past for what-if analysis or debugging, or study how context information (say, success rates of a statistical model) changes over time. \vikram{This sentence is really confusing / hard to read..}
...
bootstraps the representation of all information in the Ground metadata service \vikram{Why did we suddenly switch to saying metadata service here?}, by providing immutable objects called {\thing}s, whose evolution is captured by a graph of {\version} objects \vikram{Might want to tighten up the wording a little here? Sounds like the version history somehow spans all Items.}. Change metadata itself is the only information in Common Ground that is not versioned; this is why it forms the base of the metamodel.

% Scala traits for the \core are shown in Figure~\ref{fig:core}.  
In more detail, we begin with the definition of a \version, which is simply a globally unique identifier; it represents an immutable version of some object.
\kw{VersionHistoryDAG}s capture the change history across versions; they consist of a root \version $R$ \vikram{There's not necessarily a single root version. You can have multiple roots!}
and a set of \kw{VersionSuccessor} relations that form a DAG of {\version}s rooted at $R$ 
...
At the base of the hierarchy are identified, versioned objects; we call them {\thing}s.
A \thing consists simply of a
unique ID and a \kw{VersionHistoryDAG}. \vikram{This section is a little confusing because we start and end by talking about Items. Maybe instead, we could structure it as: There are Items, items are represneted by vhdags, and vh dags are composed of... etc?}
...
In the spirit of ``schema-on-use'', Common Ground allows diverse metadata to be independently captured as ad hoc model graphs, and integrated as needed over time.
% \begin{figure}[ht]
% \centering
...
To allow customization, the \mantle allows these objects to be associated with
...
\version of the \thing is created whenever a tag or value is changed. By contrast, {\structure}s have associated \kw{StructureVersion}s to capture their evolution; {\version}s are (optionally) associated with a specific \kw{StructureVersion}, not a \structure.
...
include GitHub repositories, or Google Docs. Ground cannot automatically track these items as they change.
...
reference (e.g., port, protocol, etc.) \vikram{We decided to remove these as standalone objects a while back. We instead just decided to have a concept of externality that was an add-on to existing objects. We should make that clear.}
...
\kw{VersionID}, an updated timestamp and possibly an updated cached value. We refer to this as a \emph{Schr\"{o}dinger} versioning scheme: each time we observe an \kw{ExternalItem} it changes. This allows Ground to track the \emph{perceived} history of an external object. \vikram{I like the phrase "Schrodinger Versioning". :-)}
...
\subsubsection{\Crust: Usage Context Graphs}
\vikram {The title's a little weird with graph in there twice.}
% \begin{figure}[ht]
% \begin{scriptsize}
% \begin{multicols}{2}
...
\kw{Principal}s (a subclass of \thing) represent the actors that work with data:  users, groups, roles, etc. 
...
optional references to \kw{Workflow} and \kw{Principal} nodes, connect (possibly differently-typed) \vikram{this is said twice} {\version}s. 
Note that \kw{LineageEdgeVersion} is not a subclass of \kw{EdgeVersion}; an \kw{EdgeVersion} can only connect two \kw{NodeVersion}s from an identical subclass \vikram{This is confusing. There's only one subclass: NodeVersion.}; a \kw{LineageEdge} can connect {\version}s from two different subclasses, including subclasses that are not under \kw{NodeVersion}.  For example, we might want to record that Sue imported \kw{nltk.py} in her \kw{churn.py} script; this is captured by a \kw{LineageEdge} between a \kw{PrincipalVersion} (representing Sue) and an \kw{EdgeVersion} (representing the dependency between the two files).  

Usage data is often generated by analyzing log files, code, and/or data, and it can become very large.
It is well known that lineage is sometimes best generated lazily on access~\cite{cheney2009provenance}; for such lazy evaluation, \emph{ephemeral} \kw{LineageEdgeVersion}s can be used to maintain the Ground lineage API without committing the (redundantly computed) \kw{LineageEdge}s to immutable storage. \vikram{I'm confused about what this is saying. It definitely doesn't exist in the code. :-)}
...
Ground's functionality is backed by five decoupled subservices.  For agility, we are starting the project using existing open source solutions for each.  We anticipate that some of these will require additional features for our purposes. In this section we go over these subservices and our initial choices of solutions, and highlight some of the research opportunities we foresee.
...
Our initial version of Ground is written in Java and implements the data model discussed in Section 3 and  provides a REST API that facilitates interaction with the system. 
...
In addition to the basics of the system, we have three external sources of data context that we have integrated with Ground:

\smallitem{Hive Metastore.} One very common type of metadata is the relational schemas that are stored in the Hive Metastore. 
As a part of our prototype, we have built a connector that allows Ground to serve as a drop-in replacement for the existing Hive Metastore. 
One major benefit of using Ground as Hive's relational catalog is that schema versioning naturally falls out of this set up. 

\smallitem{HDFS Metadata.} We have built a pipeline using Gobblin and Apache Kafka that extracts file system metadata from HDFS and writes to Kafka. 
The metadata is then ingested into Ground. 
...
\smallitem{Git Version History.} Another key kind of metadata is code versions. 
...
These three use cases combined can  provide ``time travel'' for Hive queries. 
HDFS by virtue of being an append-only file system gives us access to old versions of data via the snapshots functionality. 
Using Ground, we can access previous versions of Hive schemas as well as the code used to ETL our data. Thus, we can functionally recreate a previous state of the database.
...
Data context services are a critical missing component in today's Big Data stack. Designed properly, they should benefit not only data workers, but also researchers and 
