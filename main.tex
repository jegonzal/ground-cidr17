...
Her team warns warns her to be careful when changing these tables since other teams may depend on those fields.
% Joey talk about code versions
Upon examining the cron job, Sue realizes that these tables are constructed by applying a set of scripts that pull from other tables across the company and several of those tables are no longer being updated.
She posts on internal forms and in a few days discovers that the contents in the missing tables had been re-factored into standard units and are now exported in a new social network storage system.
...
After a few days without response, Sue pushes an updated version of the cron job and attempts to back fill the the \verb|user_fin| and \verb|ad_posts1| tables.  
