...
relational database catalogs, or scientific collaborations. \kw{StructureVersion}s allow these structures to be evolved over time in an identifiable manner.
...
\smallitem{Dwell Time Analysis.} In the vein of the analysis Sue and Janet from Section~\ref{sec:scenarios}, our second use case involves an assessment of code versions on customer behavior. 
...
\smallitem{Neo4j}. Neo4j is a native single-node graph database, so modeling the Common Ground graphs was straightforward.
