...
\author{
\alignauthor
Joseph M.\ Hellerstein\textsuperscript{*}, Vikram Sreekanti\textsuperscript{*}, Joseph E. Gonzalez\textsuperscript{*},
Shirshanka Das\textsuperscript{$\dagger$}, Akon Dey\textsuperscript{$\sharp$}, Mark Donsky\textsuperscript{$\ddagger$}, Sreyashi Nag\textsuperscript{$\S$}, Krishna Ramchandran\textsuperscript{$\natural$}, Chang She\textsuperscript{$\ddagger$}, Carl Steinbach\textsuperscript{$\dagger$}\\
{\small
...
challenge to the data management community: how do we manage \emph{data context}?
Capturing the data context --- the origin of data, how it is changing, how it is being used, who is using it, and for what purposes --- is essential to both maximizing the value of data and also minimizing the risk. 
Managing data context is in many ways more challenging than managing the data itself as context depends heavily on the types and usages of data, spans multiple systems and organization roles, and can evolve faster and grow larger than the underlying data.

% Take credit for framing the problem!
In this paper we frame the challenges of managing data context along three axis: \emph{Application}, \emph{Behavior}, and \emph{Change}. 
% Take credit for proposing a solution 
Using these axis we introduce Ground an open-source \emph{data context management system} to manage all the peripheral information that informs the use of data. 
% Describe how solution connects to framing
We describe an early prototype of Ground that exposes a narrow waist context service built on-top of a highly expressive graph data model. 
We present early experimental results on our prototype implementation of Ground using realistic benchmark workloads and outline some of the remaining challenges that need to be solved.
...
For example, an agent can subscribe for notifications of file data, hand off the files to an entity extraction system like OpenCalais or DeepDive, and subsequently tag the corresponding Common Ground metadata objects with tags for the extracted entities.
...
Identity management and authorization are required for a context service. Interoperability is important here, as most organizations dictate previously-chosen packages.  
Note that authorization needs vary widely: the policies of a scientific consortium will differ from a national defense agency or a corporate marketing department.  
Ground's flexible metamodel can support a variety of relevant metadata (ownership, content labels, etc.) Ground's authorization APIs can be set synchronous to support enforcement policies, and/or asynchronous for post-hoc auditing.
...
The virtues of immutability clearly depend on the use case. More research is needed to 
understand how versions and lineage 
interact with security techniques like Information Flow Control~\cite{zeldovich2008securing} in the context of evolving real-world pipelines.
...
The \core allows for ad hoc {\gtag}s, but many applications model information more carefully.
To that end, the \mantle includes a subclass of \thing called {\structure}. A \structure is like a schema or C struct: a set of {\gtag}s that must be present, to enforce structural constraints. Unlike database schemas or C structs, the \structure class of Ground is explicitly versioned, via a \kw{StructureVersion} subclass in the \core.  An \thing can be optionally associated with a \structure; if so, each \version of the \thing is associated with a corresponding \kw{StructureVersion} and must define those {\gtag}s (along, optionally, with other ad hoc {\gtag}s.)
...
(a workflow) or manual (via some principal). \kw{LineageEdgeVersion}s (dark arrows in the top layer of Figure~\ref{fig:layers}) connect two or more (possibly differently-typed) {\version}s, one of which is a \kw{Workflow} or \kw{Principal} nodes. 
...
Hence, we expect Aboveground clients to define custom \kw{Structure}s to capture their application semantics.  
These can be packaged under library {\node}s for sharing---e.g. a library for representing
relational database catalogs, or scientific collaborations, or the models of a typical application like Catly in Section~\ref{sec:scenarios}. \kw{StructureVersion}s allow these structures to be evolved over time in an identifiable manner.
...
To exercise our initial design and provide useful functionality ``out of the box'', we provide support for three sources of metadata most commonly used in the Big Data ecosystem: file metadata from HDFS, schemas from Hive, and code versioning from Git.
To support HDFS, we extended Gobblin to extract file system metadata from its HDFS crawls, and publish them to Ground's Kafka connector. The resulting metadata is then ingested into Ground, and notifications are published on a Kafka channel for applications to respond to. To support Hive, we built an API shim that allows Ground to serve as a drop-in replacement for Hive Metastore.
...
As a small real-world model for this use case, we took the source code of Apache Hadoop, and constructed a dependency graph of file imports
that we register in Ground.
We perform impact analysis by running transitive closure starting from a particular version of a file and retrieving every other file version on which it depends.

\smallitem{Dwell Time Analysis.} In the vein of the analysis Sue and Janet were doing in Section 2, our second use case involves
a standard assessment of code versions on customer behavior. In this case, we study how user ``dwell time'' on a web page correlates with the version of the software that populates the page (e.g. with news stories). We were able to get access to sizable real-world weblogs~\cite{starwarskid}, but had to simulate versions of a content-selection pipeline. To that end we took version history from the git logs of the Apache httpd web server.  Our experiment breaks the Star Wars Kid web log into sessions, and maps each session to a version of the software.
We then join each session with information on the software version it used, in order to enable dwell-time analysis. 
\\

While both these use cases are less than realistic both in scale and in actual functionality, we felt they could provide simple lower-bound
results for more complex use cases.
...
Our initial goal here was more experiential than quantitative---we wanted to see if we could easily get these systems to 
perform adequately for our use cases, and if not to call more attention to the needs of a system like Ground.
We acknowledge that with further tuning, these systems might perform better than they did in our experiments, though we feel 
these experiments
are rather fundamental and should not require intensive tuning.
...
(e.g., \kw{EdgeVersion}) as their own binary tables with indexes on both sides. 
\jmh{What is ``adjacency retrieval''? This next sentence worries me.}
Hence adjacency retrieval is a single-table look-up that is optimized with an index. 
The performance results here are expected and not noteworthy.

For the dwell time experiment, we compared three different implementations in PostgreSQL. The first was a recursive query using the \kw{with} syntax. 
The second was a UDF written in PGPLSQL that iteratively (semi-naïvely) computed the paths that we were interested in. 
The last was a fully unrolled \jmh{8-way?} self-join that manually computed the paths of the longest possible length. Figure~\ref{fig:impact} shows that were unable to get Postgres within an order of magnitude of viable graph processing. \jmh{are you planning to plot all 3?}

\smallitem{Cassandra.} In Cassandra, every entity and relationship from the Common Ground model is represented as a key/value pair, indexed
by key.
...
Cassandra can be integrated with an in-memory Java graph-processing library called JGraphT to answer the query. To give Cassandra every advantage we utilized JGraphT; Figure~\ref{fig:impact} shows this providing a baseline for what's possible using off-the-shelf open source for this simple graph query.


\smallitem{Neo4j} Neo4j is a native single-node graph database, so we modeled the graphs in Common Ground in a straightforward manner.
...
Neo4j excelled in the transitive closure scenario, performing as fast as the Java in-memory implementation. \jmh{That's not what the graph shows!}
...
\smallitem{TitanDB.} TitanDB is a scale-out graph databases designed to run over a NoSQL database like Cassandra, which is how we deployed it in our experiments. \jmh{True?} TitanDB's adjacent query performance was ... \joey{finish this!}
...
LinkedIn WhereHows and Google Goods \jmh{to finish}.  
...
In future work we plan to assess this space more deeply. \joey{not sure what we mean by assess this space?  Perhaps we want to say: In the future we plan to revisit many ideas (e.g., ) in these earlier systems as we develop systems for metadata management.}
