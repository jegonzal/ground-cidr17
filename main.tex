...
\joey{I am taking a pass on this now (working offline and will push to git).}
Sue, a data-scientist at a major online social media organization, recently joined their news feed team to develop a more personalized targeting system.
...
Sue, curious about how these tables are populated, asks her team members who tell her about the intern that created the original cron jobs and warn her to be careful when changing the input tables since other teams may depend on those fields.
% Joey talk about code versions
Upon examining the cron job, Sue realizes that these tables are constructed by applying a set of scripts that pull from other tables across companies data infrastructure.
She posts on internal forms and discovers that several of the tables are no longer being updated and realizes that she can improve performance by backfilling these tables.
However before doing this she also needs to check that the resulting changes do not interfere with any of the other users that currently depend on these tables. 
...
and realizes that the content on the news feed is populated from a single content table which contains articles about each post along with fields determine who 


She consults the meta-data system for the web front-end service and sees that the ranking system relies on the content-score field associated with each record.  


The current targeting system takes pulls content from advertisers, other users, and media partners which are outputs of multiple other systems at her organization.  
Each of these pipelines annotates the content with important descriptors which Sue hopes to use in her new pipeline. 
