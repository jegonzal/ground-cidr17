...
Machine learning powered service from content recommendation to risk management and fraud detection are quickly becoming common place.
These services critically depend on up-to-date data to train effective models.
Often this data is derived from multiple sources (e.g., click streams, content catalogs, and purchase histories).
We believe that by connecting commonly used modeling frameworks (e.g., scikit-learn and TensorFlow) to ground we will be able to better identify the correct data to train models and automatically update models as data changes. 
Furthermore, by registering models directly within ground, we will be able to identify and track data dependencies and address many of the problems with machine learning in production outlined by Sculley et al. in \cite{sculley2}.
